...
'            ReadTBL("RCBSE", sSQL, false).Flush
        ...
        Case "GENPDF" 'Generación del contrato en PDF
            ...
            'pC=Request.QueryString("C")
            pN=Replace(Replace(Request.QueryString("N"),"%20","_")," ","_")
            ...
            oJSON("cmd")="cmd.exe /c """ & Request.ServerVariables("APPL_PHYSICAL_PATH")  & "\cnd\cndengine.vbs " & pVC & " " & Session.SessionID & " " & pN & """ "
            AddLog "[GenPDF] cmd.exe /c """ & Request.ServerVariables("APPL_PHYSICAL_PATH")  & "\cnd\cndengine.vbs " & pVC & " " & Session.SessionID & " " & pN & """ "
            log "[GenPDF] cmd: cmd.exe /c """ & Request.ServerVariables("APPL_PHYSICAL_PATH")  & "\cnd\cndengine.vbs " & pVC & " " & Session.SessionID & " " & pN & """ "
            sRtn = oWSH.Run ("cmd.exe /c """ & Request.ServerVariables("APPL_PHYSICAL_PATH")  & "\cnd\cndengine.vbs " & pVC & " " & Session.SessionID & " " & pN & """ ", 0, true)
            oJSON("cmdReturn") = sRtn
